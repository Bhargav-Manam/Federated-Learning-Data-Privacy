...
def by_labels_non_iid_split(dataset, n_classes, n_clients, n_clusters, alpha, frac, rng=None):
    ...
    for group_idx, labels in enumerate(clusters_labels):
        for label in labels:
            label2cluster[int(label)] = group_idx
...
def pathological_non_iid_split(dataset, n_classes, n_clients, n_classes_per_client, frac=1, rng=None):
    ...
    for idx in selected_indices:
        label = dataset.targets[idx]
        label2index[int(label)].append(idx)

    sorted_indices = []
    for label in label2index:
        sorted_indices += label2index[int(label)]
...
def generate_data(
    ...
    if split_type == "pathological_split":
        ...
        print(f"==> Data are split by labels (non-IID), with alpha={alpha}")
        clients_indices = \
    ...
    for client_id in tqdm(range(n_clients), total=n_clients, desc="Clients.."):
        ...
        test_indices = client_indices[client_indices < n_train_samples//2]
        ...
        if len(train_targets) == 0:
            raise RuntimeError(f"Client {client_id} has no train data!")
        if len(test_targets) == 0:
            raise RuntimeError(f"Client {client_id} has no test data!")
