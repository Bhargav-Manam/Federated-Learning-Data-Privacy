...
if __name__ == "__main__":
    ...
    print("\n=> Build aggregator..")
    aggregator, attacker = build_experiment(args_=args, seed_=seed)
    ...
    for ii in tqdm(range(args.n_rounds)):
        ...
        if (ii % args.log_freq) == (args.log_freq - 1):
            aggregator.write_logs()
            #for attack at multiple rounds
            #model = aggregator.global_trainer.model
            #attacker.model_inversion_attack(model, n_rounds=args.n_attack_rounds)
            #attacker.write_attack_logs(c_round=ii, plot=False)

    #for attack only after final round
    model = aggregator.global_trainer.model
    attacker.model_inversion_attack(model, n_rounds=args.n_attack_rounds)
    attacker.write_attack_logs(c_round=args.n_rounds, plot=False)
